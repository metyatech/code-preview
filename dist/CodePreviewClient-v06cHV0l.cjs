...
const useGlobalSourceSync = ({ sourceId, store, setHtmlCode, setCssCode, setJsCode, hasInitialHTML, hasInitialCSS, hasInitialJS, initialStateRef }) => {
    const capturedInitialRef = React.useRef({
...
const usePathname = () => React.useSyncExternalStore(subscribe, getSnapshot, getServerSnapshot);
...
const useSourceCodeStore = (props) => {
    ...
    const scopedSourceId = React.useMemo(() => {
        if (!sourceId)
            return undefined;
        if (!pathname)
            return sourceId;
    ...
    const storedState = React.useSyncExternalStore(subscribe, getSnapshot, () => undefined);
    ...
    const [htmlCode, setHtmlCode] = React.useState(ensureTrailingNewline(resolvedHTML || ''));
    const [cssCode, setCssCode] = React.useState(ensureTrailingNewline(resolvedCSS || ''));
    const [jsCode, setJsCode] = React.useState(ensureTrailingNewline(resolvedJS || ''));
    ...
    const resetCodes = React.useCallback(() => {
...
const useEditorResize = ({ resizeTargets, containerRef, initialWidths }) => {
    const [sectionWidths, setSectionWidths] = React.useState(() => {
        if (initialWidths)
    ...
    const calculateOptimalWidths = React.useCallback(() => {
    ...
    const updateSectionWidths = React.useCallback((force = false) => {
    ...
    const resetSectionWidthsToAuto = React.useCallback(() => {
...
const useEditorHeight = ({ minHeightPx, editors }) => {
    const [editorHeight, setEditorHeight] = React.useState(`${minHeightPx}px`);
    const calculateEditorHeight = React.useCallback(() => {
...
const usePreviewHeight = ({ minHeightPx, showPreview, iframeRef, editors }) => {
    const [previewHeight, setPreviewHeight] = React.useState(`${minHeightPx}px`);
    // Track the maximum height ever reached (only grows, never shrinks)
    const maxHeightRef = React.useRef(minHeightPx);
    // Reset max height when code changes (new iframe content)
    const resetMaxHeight = React.useCallback(() => {
        maxHeightRef.current = minHeightPx;
    }, [minHeightPx]);
    const calculatePreviewHeight = React.useCallback(() => {
    ...
    const requestPreviewHeight = React.useCallback(() => {
        const iframe = iframeRef.current;
    ...
    const updatePreviewHeight = React.useCallback(() => {
        if (!showPreview)
            return;
        setTimeout(() => {
...
const useResetHandler = ({ onReset, longPressDuration = 500 }) => {
    ...
    const resetProgressIntervalRef = React.useRef(null);
    /**
     * タイマーとインターバルをクリアする
     */
     ...
     */
    const handleResetMouseDown = React.useCallback(() => {
     ...
     */
    const handleResetMouseUp = React.useCallback(() => {
        clearTimers();
        setResetProgress(0);
    }, [clearTimers]);
    /**
     ...
     */
    const handleResetMouseLeave = React.useCallback(() => {
        clearTimers();
        setResetProgress(0);
    }, [clearTimers]);
...
const useEditorMount = (updateSectionWidths) => {
    const createMountHandler = React.useCallback((ref) => {
...
const useEditorConfigs = ({ editors, updateSectionWidths }) => {
    ...
    // 設定の生成
    const visibleEditorConfigs = React.useMemo(() => {
        return editors
            .filter((editor) => editor.visible)
            .map((editor) => ({
            key: editor.key,
...
const useCodePreview = (props) => {
    ...
    const editorsRowRef = React.useRef(null);
    ...
    const [isToggledManually, setIsToggledManually] = React.useState(false);
    const [manualShowValue, setManualShowValue] = React.useState(false);
    const [lastFileStructureVisible, setLastFileStructureVisible] = React.useState(fileStructureVisible);
    ...
    const showFileStructure = React.useMemo(() => {
    ...
    const rawIframeId = React.useId();
    const iframeId = React.useMemo(() => `iframe-${rawIframeId.replace(/:/g, '')}`, [rawIframeId]);
    ...
    const toggleFileStructure = React.useCallback(() => {
    ...
    const remountIframe = React.useCallback(() => {
...
const FileStructurePanel = ({ resolvedHtmlPath, resolvedCssPath, resolvedJsPath, resolvedImages }) => {
    const { folders, rootFiles } = React.useMemo(() => buildFileStructure(resolvedHtmlPath, resolvedCssPath, resolvedJsPath, resolvedImages), [resolvedHtmlPath, resolvedCssPath, resolvedJsPath, resolvedImages]);
    return (jsxRuntime.jsxs("div", { className: styles.fileStructure, children: [jsxRuntime.jsx("div", { className: styles.fileStructureTitle, children: "\uD83D\uDCC1 \u30D5\u30A1\u30A4\u30EB\u69CB\u9020" }), jsxRuntime.jsxs("div", { className: styles.fileTree, children: [rootFiles.map((file) => (jsxRuntime.jsxs("div", { className: styles.fileTreeItem, children: [jsxRuntime.jsx("span", { className: styles.fileIcon, children: "\uD83D\uDCC4" }), " ", file] }, file))), Array.from(folders.entries()).map(([folderPath, files]) => (jsxRuntime.jsxs("div", { className: styles.fileTreeFolder, children: [jsxRuntime.jsxs("div", { className: styles.fileTreeItem, children: [jsxRuntime.jsx("span", { className: styles.folderIcon, children: "\uD83D\uDCC1" }), " ", folderPath] }), files.map((file) => (jsxRuntime.jsxs("div", { className: styles.fileTreeSubItem, children: [jsxRuntime.jsx("span", { className: styles.fileIcon, children: "\uD83D\uDCC4" }), " ", file] }, `${folderPath}/${file}`)))] }, folderPath)))] })] }));
};

const ConsolePanel = ({ logs }) => {
    return (jsxRuntime.jsxs("div", { className: styles.consoleSection, children: [jsxRuntime.jsx("div", { className: styles.sectionHeader, children: "\u30B3\u30F3\u30BD\u30FC\u30EB" }), jsxRuntime.jsx("div", { className: styles.consoleContainer, children: logs.length === 0 ? (jsxRuntime.jsx("div", { className: styles.consolePlaceholder, children: "\u3053\u3053\u306B console.log \u306E\u7D50\u679C\u304C\u8868\u793A\u3055\u308C\u307E\u3059" })) : (logs.map((log, index) => (jsxRuntime.jsxs("div", { className: styles.consoleLine, children: [jsxRuntime.jsx("span", { className: styles.consoleBullet, children: "\u25B6" }), jsxRuntime.jsx("span", { children: log })] }, index)))) })] }));
};

const ResetButton = ({ resetProgress, onMouseDown, onMouseUp, onMouseLeave }) => {
    return (jsxRuntime.jsxs("button", { type: "button", className: styles.gyoButton, onMouseDown: onMouseDown, onMouseUp: onMouseUp, onMouseLeave: onMouseLeave, onTouchStart: onMouseDown, onTouchEnd: onMouseUp, title: "\u9577\u62BC\u3057\u3067\u30EA\u30BB\u30C3\u30C8", children: [jsxRuntime.jsx("span", { className: styles.resetProgressCircle + (resetProgress > 0 ? ' ' + styles.isCharging : ''), "aria-hidden": "true", children: jsxRuntime.jsxs("svg", { width: "24", height: "24", viewBox: "0 0 24 24", children: [resetProgress > 0 && (jsxRuntime.jsx("circle", { cx: "12", cy: "12", r: "10", fill: "none", stroke: "#218bff", strokeWidth: "2.2", strokeLinecap: "round", strokeDasharray: 2 * Math.PI * 10, strokeDashoffset: (1 - resetProgress) * 2 * Math.PI * 10, style: { transition: 'stroke-dashoffset 0.05s linear' } })), jsxRuntime.jsxs("g", { children: [jsxRuntime.jsx("path", { d: "M12 5a7 7 0 1 1-5.3 2.7", fill: "none", stroke: "#218bff", strokeWidth: "2.2", strokeLinecap: "round", strokeLinejoin: "round" }), jsxRuntime.jsx("polyline", { points: "6.5,7.5 6.5,4.5 9.5,4.5", fill: "none", stroke: "#218bff", strokeWidth: "2.2", strokeLinecap: "round", strokeLinejoin: "round" })] })] }) }), jsxRuntime.jsx("span", { className: styles.hiddenText, children: "\u9577\u62BC\u3057\u3067\u30EA\u30BB\u30C3\u30C8" })] }));
};

const ToolbarButton = ({ onClick, pressed, label, icon }) => {
    return (jsxRuntime.jsxs("button", { type: "button", className: styles.gyoButton, onClick: onClick, "aria-pressed": pressed, title: label, children: [jsxRuntime.jsx("span", { "aria-hidden": "true", children: icon }), jsxRuntime.jsx("span", { className: styles.hiddenText, children: label })] }));
};
...
const Toolbar = ({ resetProgress, showLineNumbers, showFileStructure, onResetMouseDown, onResetMouseUp, onResetMouseLeave, onToggleLineNumbers, onToggleFileStructure }) => {
    return (jsxRuntime.jsxs(jsxRuntime.Fragment, { children: [jsxRuntime.jsx(ResetButton, { resetProgress: resetProgress, onMouseDown: onResetMouseDown, onMouseUp: onResetMouseUp, onMouseLeave: onResetMouseLeave }), jsxRuntime.jsx(ToolbarButton, { onClick: onToggleLineNumbers, pressed: showLineNumbers, label: showLineNumbers ? '行番号を隠す' : '行番号を表示', icon: "#" }), jsxRuntime.jsx(ToolbarButton, { onClick: onToggleFileStructure, pressed: showFileStructure, label: showFileStructure ? 'ファイル構造を隠す' : 'ファイル構造を表示', icon: "\uD83D\uDCC1" })] }));
};
...
const EditorPanel = ({ config, width, height, theme, showLineNumbers }) => {
    ...
    return (jsxRuntime.jsxs("div", { className: styles.editorSection, style: { width: `${width}%` }, children: [jsxRuntime.jsx("div", { className: styles.sectionHeader, children: config.label }), jsxRuntime.jsx("div", { className: styles.editorContainer, children: jsxRuntime.jsx(Editor, { height: height, defaultLanguage: config.language, value: config.value, onChange: config.onChange, onMount: config.onMount, theme: theme, options: mergedOptions }) })] }));
};

const Resizer = ({ leftKey, rightKey, leftLabel, rightLabel, onMouseDown, onKeyDown, onDoubleClick }) => {
    return (jsxRuntime.jsx("div", { className: styles.resizer, role: "separator", "aria-orientation": "vertical", "aria-label": `${leftLabel} と ${rightLabel} の幅を調整`, tabIndex: 0, onMouseDown: (event) => onMouseDown(event, leftKey, rightKey), onKeyDown: (event) => onKeyDown(event, leftKey, rightKey), onDoubleClick: onDoubleClick, children: jsxRuntime.jsx("span", { className: styles.resizerGrip }) }));
};
...
const EditorSection = ({ layout, state, handlers, editorsRowRef, editorsRowStyle }) => {
    ...
    return (jsxRuntime.jsxs("div", { className: editorsRowClassName, style: editorsRowStyle, ref: editorsRowRef, children: [jsxRuntime.jsx(Toolbar, { resetProgress: handlers.resetProgress, showLineNumbers: layout.showLineNumbers, showFileStructure: state.showFileStructure, onResetMouseDown: handlers.handleResetMouseDown, onResetMouseUp: handlers.handleResetMouseUp, onResetMouseLeave: handlers.handleResetMouseLeave, onToggleLineNumbers: handlers.toggleLineNumbers, onToggleFileStructure: handlers.toggleFileStructure }), layout.visibleEditorConfigs.map((config, index) => {
                const nextConfig = layout.visibleEditorConfigs[index + 1];
                return (jsxRuntime.jsxs(React.Fragment, { children: [jsxRuntime.jsx(EditorPanel, { config: config, width: layout.sectionWidths[config.key], height: layout.editorHeight, theme: state.editorTheme, showLineNumbers: layout.showLineNumbers }), nextConfig ? (jsxRuntime.jsx(Resizer, { leftKey: config.key, rightKey: nextConfig.key, leftLabel: config.label, rightLabel: nextConfig.label, onMouseDown: handlers.handleMouseDown, onKeyDown: handlers.handleResizerKeyDown, onDoubleClick: (event) => {
                                event.preventDefault();
...
const PreviewPanel = ({ iframeRef, iframeKey, previewHeight, minHeightCss, visible, generatorOptions }) => {
    const loadedOnClientRef = React.useRef(false);
    React.useEffect(() => {
        if (!visible)
            return;
        const iframe = iframeRef.current;
    ...
    return (jsxRuntime.jsx("iframe", { ref: iframeRef, srcDoc: generatePreviewDocument(generatorOptions), onLoad: () => {
...
const PreviewSection = ({ visibility, state, layout, minHeightCss, cssPath, jsPath, iframeRef }) => {
    ...
    return (jsxRuntime.jsxs("div", { className: styles.previewSection, style: { display: visibility.showPreview ? 'flex' : 'none' }, children: [jsxRuntime.jsx("div", { className: styles.sectionHeader, children: "\u30D7\u30EC\u30D3\u30E5\u30FC" }), jsxRuntime.jsx("div", { className: styles.previewContainer, children: jsxRuntime.jsx(PreviewPanel, { iframeRef: iframeRef, iframeKey: state.iframeKey, previewHeight: layout.previewHeight, minHeightCss: minHeightCss, visible: true, generatorOptions: {
...
const CodePreviewLayout = ({ elementRefs, state, visibility, layout, handlers, title, cssPath, jsPath }) => {
    ...
    return (jsxRuntime.jsxs("div", { className: styles.codePreviewContainer, children: [title ? (jsxRuntime.jsx("div", { className: styles.header, children: jsxRuntime.jsx("h4", { className: styles.title, children: title }) })) : null, jsxRuntime.jsxs("div", { className: styles.splitLayout, ref: containerRef, style: splitLayoutStyle, children: [state.showFileStructure && (jsxRuntime.jsx(FileStructurePanel, { resolvedHtmlPath: state.resolvedHtmlPath, resolvedCssPath: state.resolvedCssPath, resolvedJsPath: state.resolvedJsPath, resolvedImages: state.resolvedImages })), jsxRuntime.jsx(EditorSection, { layout: layout, state: state, handlers: handlers, editorsRowRef: editorsRowRef, editorsRowStyle: editorsRowStyle }), jsxRuntime.jsx(PreviewSection, { visibility: visibility, state: state, layout: layout, minHeightCss: layout.minHeightCss, cssPath: cssPath, jsPath: jsPath, iframeRef: iframeRef }), visibility.showConsole && jsxRuntime.jsx(ConsolePanel, { logs: state.consoleLogs })] })] }));
};
...
function CodePreviewClient(props) {
    ...
    const parsedSource = React.useMemo(() => {
        if (!CodePreviewShared.shouldParseCodeBlocksFromChildren(children, initialHTML, initialCSS, initialJS)) {
            return {};
        }
        return CodePreviewShared.parseCodeBlocksFromChildren(children);
    ...
    return jsxRuntime.jsx(CodePreviewLayout, { ...hookResult, title: title, cssPath: cssPath, jsPath: jsPath });
}

exports.CodePreviewClient = CodePreviewClient;
//# sourceMappingURL=CodePreviewClient-v06cHV0l.cjs.map
