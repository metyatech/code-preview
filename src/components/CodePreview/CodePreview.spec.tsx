...
test.describe("CodePreview コンポーネントのテスト", () => {
  ...
  test("initialHTML overrides fenced content when specified", async ({ mount }) => {
    ...
    const component = await mount(
      <CodePreview initialHTML="<div id='from-prop'>Prop</div>">{raw}</CodePreview>,
    );
  ...
  test("initialCSS overrides fenced content when specified", async ({ mount }) => {
    ...
    const component = await mount(
      <CodePreview initialCSS="#color-box { color: blue; }">{raw}</CodePreview>,
    );
  ...
  test("タイトルが指定された場合、正しく表示されること", async ({ mount }) => {
    const component = await mount(
      <CodePreviewFixture title="テスト用タイトル" html="<div>Test</div>" />,
    );
  ...
  test("ファイル構造パネルの表示切り替えができること", async ({ mount }) => {
    const component = await mount(
      <CodePreviewFixture fileStructureVisible={true} html="<div></div>" />,
    );
  ...
  test("プレビュー（iframe）内にコンテンツが描画されること", async ({ mount }) => {
    const component = await mount(
      <CodePreviewFixture html="<div id='test-target'>Hello World</div>" />,
    );
  ...
  test("HTMLエディタを非表示にしてもHTMLがある場合はプレビューが表示されること", async ({
    mount,
  }) => {
  ...
  test("htmlPathのデフォルト値(index.html)が適用されること", async ({ mount }) => {
    const component = await mount(
      <CodePreviewFixture fileStructureVisible={true} html="<div>test</div>" />,
    );
  ...
  test("ファイル構造の初期状態がfalseの場合、非表示から開始されること", async ({ mount }) => {
    const component = await mount(
      <CodePreviewFixture fileStructureVisible={false} html="<div>test</div>" />,
    );
  ...
  test("不正なCSSでもクラッシュしないこと", async ({ mount }) => {
    const component = await mount(
      <CodePreviewFixture html="<div>test</div>" css="div { color: red" />,
    );
  ...
  test("imagesが空オブジェクトの場合は共有ストアのimagesをクリアできること", async ({
    mount,
    page,
  }) => {
    await page.evaluate(() => {
  ...
  test("sourceId共有時、JSが提供されていない場合はJavaScriptエディタが表示されないこと", async ({
    mount,
  }) => {
    const raw = [
      "```html",
  ...
  test("初期コードに共通インデントがない場合は先頭スペースが保持されること", async ({
    mount,
    page,
  }) => {
    const rawHtml = "\nA\n  B\n";
...
`}
  ...
  test("iframeIdが一致しなくても同一iframeからの高さ通知で更新されること", async ({ mount }) => {
    const component = await mount(
      <CodePreviewFixture html="<div>Short content</div>" minHeight="100px" />,
    );
    ...
    await frame.locator("body").evaluate(() => {
      window.parent.postMessage(
        { type: "codePreviewHeightChange", height: 420, iframeId: "mismatch-id" },
        "*",
      );
    });
  ...
  test("モーダルウィンドウのような固定配置要素が表示された場合、高さが広がること", async ({
    mount,
  }) => {
    const component = await mount(
      <CodePreviewFixture
...
<button id="shrink-btn" onclick="document.getElementById('content').style.height = '50px';">縮小</button>`}
    ...
    const heightBeforeShrink = await iframe.evaluate(
...
`}
  ...
  test("cssPathが指定されている場合、fileStructureVisibleが未指定でも表示されること", async ({
    mount,
  }) => {
    const component = await mount(
      <CodePreviewFixture
  ...
  test("jsPathが指定されている場合、fileStructureVisibleが未指定でも表示されること", async ({
    mount,
  }) => {
    const component = await mount(
  ...
  test("imagesが指定されていても、fileStructureVisible=falseなら非表示であること", async ({
    mount,
  }) => {
    const component = await mount(
      <CodePreviewFixture
